...
module.exports = {
    async before(m) {
        ...
        if (!isSurrender) {
            let index = room.result.jawaban.findIndex(v => v.toLowerCase().replace(/[^\w\s\-]+/, '') === text)
            if (index < 0) {
                if (Math.max(...room.result.jawaban.filter((_, index) => !room.terjawab[index]).map(jawaban => similarity(jawaban, text))) >= threshold) m.reply(dikit)
                return !0
            }
