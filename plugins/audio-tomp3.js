...
let handler = async (m, { conn, usedPrefix, command }) => {
  ...
  if (/mp3|a(udio)?$/i.test(command)) {
    ...
    await conn.sendFile(m.chat, audio, '', '', m, 0, { mimetype: 'audio/mp4', asDocument: db.data.chats[m.chat].useDocument })
  }
  if (/vn|ptt$/i.test(command)) {
    if (!/video|audio/.test(mime)) throw `balas video/audio dengan perintah ${usedPrefix + command}`
    let media = await q.download()
    let audio = await toPTT(media, 'mp4')
    await conn.sendFile(m.chat, audio, '', '', m, 1, { mimetype: 'audio/mp4' })
  }
}
